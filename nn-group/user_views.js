...
export async function mountSavedViews(opts) {
  ...
  async function loadFromDB() {
    ...
    cache = (data || []).map(r => ({ id: r.id, name: r.name, filters: r.filters || {}, isDefault: !!r.is_default }))
  }
  ...
  function render() {
    ...
    for (const v of cache) {
      ...
      html += `<button type="button" class="sv-chip ${isOn ? 'active' : ''} ${isOn && unsaved ? 'unsaved' : ''}" data-action="apply" data-id="${v.id}">
        <span>${v.isDefault ? '⭐ ' : ''}${_esc(v.name)}</span>
        <span class="sv-chip-actions">
          <button type="button" class="sv-act" data-action="default" data-id="${v.id}" title="${v.isDefault ? 'Unset default (stop auto-loading)' : 'Set as default — auto-loads when you open this page'}">${v.isDefault ? '⭐' : '☆'}</button>
  ...
  async function toggleDefault(id) {
    ...
    // Clear any existing default for this page (owner-scoped by RLS), then set.
    const { error: e1 } = await sb.from('user_views').update({ is_default: false }).eq('page', page).eq('is_default', true)
    if (e1) { ntf('Could not update default: ' + e1.message, 'err'); return }
    if (makeDefault) {
      const { error: e2 } = await sb.from('user_views').update({ is_default: true }).eq('id', id)
      if (e2) { ntf('Could not set default: ' + e2.message, 'err'); return }
    }
    cache.forEach(x => { x.isDefault = false }); v.isDefault = makeDefault
    render()
    ntf(makeDefault ? `"${v.name}" will now load automatically when you open this page` : 'Default view cleared', 'ok')
  }
  ...
  row.addEventListener('click', e => {
    ...
    else if (action === 'default')  toggleDefault(id)
  ...
  // A pinned default view (DB, cross-device) wins on load — that's what makes
  // the page open straight onto the view the user chose, even on mobile.
  // Otherwise we fall back to the last-used view remembered on this device.
  render()
  await loadFromDB()
  await migrateLegacy()
  const def = cache.find(v => v.isDefault)
  if (def) {
    currentViewId = def.id; localStorage.setItem(currentViewKey, def.id)
    writeFilters(def.filters)
